...
egen  pid = concat(hhid S1AQ00), punct(-)
sort  hhid pid
...
datalibweb, country(BGD) year(2022) type(SARRAW) surveyid(BGD_2022_HIES_v01_M) filename(HH_SEC_7A.dta) clear 
duplicates report PSU HHID
egen  hhid = concat(PSU HHID), punct(-)
sort  hhid
...
* LANDHOLDING
tempfile landholding
use "$rootdatalib\\`code'\\`yearfolder'\\`yearfolder'_v`vm'_M\Data\Stata\HH_SEC_7A.dta", clear 
duplicates report PSU HHID
egen  hhid = concat(PSU HHID), punct(-)
sort  hhid
save `landholding', replace
...
use  `IND', clear
merge 1:1 hhid pid using `ind_roster', keep(match)
drop _merge
sort  hhid
merge m:1 hhid using `housing', keep(match)
drop _merge
sort  hhid
merge m:1 hhid using `landholding', keep(match)
drop _merge
sort  hhid
