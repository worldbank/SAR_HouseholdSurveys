...
tempfile INC
*datalibweb, country(BGD) year(2022) type(SARMD) survey(HIES) vermast(01) veralt(01) mod(INC) clear 
use "$rootdatalib\\`code'\\`yearfolder'\\`SARMDfolder'\Data\Harmonized\\`yearfolder'_v`vm'_M_v`va'_A_SARMD_INC.dta", clear
drop  weighttype
sort  hhid pid
save `INC', replace
...
egen  pid = concat(hhid S4AQ00), punct(-)
sort  hhid pid
save `EMP', replace
*/
...
* EMPLOYMENT
tempfile EMP
use "D:\Dropbox2\datalib\SARMD\WORKINGDATA\BGD\BGD_2022_HIES\BGD_2022_HIES_v01_M\Data\Stata\HH_SEC_4A.dta", clear 
collapse (sum) S4AQ02, by(PSU HHID S4AQ00)
egen  hhid = concat(PSU HHID), punct(-)
egen  pid = concat(hhid S4AQ00), punct(-)
...
use  `IND', clear
merge 1:1 hhid pid using `INC', keep(match)
drop _merge
sort  hhid pid
merge 1:1 hhid pid using `EMP'
drop _merge
sort  hhid pid
*</_Datalibweb request_>
...
egen wage_nc_2_year = rsum(daylab_cash_2 employee_cash_2 agri_net_2 month_nonagri_3), missing
*</_wage_nc_2_year_>
